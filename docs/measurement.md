...
the fixed metric because they always make a valid-but-different edit. That is a real property of the
model, confirmed by auditing predictions four ways, not a leftover measurement bug. The fixed metric
...
1. **Truncating exemplars.** The few-shot prompt is built by concatenating retrieved examples cut to
   ...
   That slices mid-statement, sometimes mid-identifier, on any method longer than the cap (CodeXGLUE
