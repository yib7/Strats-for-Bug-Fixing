...
| **Weights & Biases** | `pop pretrain`, `pop finetune`, `pop lora`, and only when `WANDB_API_KEY` is set | Training metrics, to *your own* W&B account | Leave `WANDB_API_KEY` unset (the default), or set `WANDB_MODE=offline` |
