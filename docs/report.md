...
> oversell — and here the two lenses **disagree** with each other, which turns out to be the
...
finetuned) beat the alternatives for Java bug-fixing — namely finetuning the **same small model
from a random init**, **prompting a larger code LLM** with retrieval (RAG), or **cheaply adapting**
that larger LLM with LoRA? Four arms, one held-out benchmark, two lenses (surface similarity via
CodeBLEU; does-it-actually-run via execution pass@1).
...
prompt can make RAG score *worse* than zero-shot — both measurement artifacts, not real findings
...
| **B (mean ± band)** | — | — | **0.479** (0.476–0.484) | **0.885** (0.878–0.895) | 6545 |
...
Best config is **codebert_k1** (CodeBLEU 0.6517, syntax-valid 0.960, EM 0.0189) — this is the arm-C bar
...
1.5B Qwen base scores **CodeBLEU 0.8543, syntax-valid 0.9366, EM 0.0947** (`results/lora_qwen_test.json`)
— the best arm on both CodeBLEU and EM by a wide margin: +0.20 CodeBLEU over arm C's best RAG config
and +0.38 over the T5 arms, with an EM rate ~5x arm C's best (9.47% vs 1.89%) and far above the T5
arms' ~0%. Cheap parameter-efficient adaptation of a capable base model dominates every other arm on
the surface-similarity lens — though this ranking does **not** survive the execution lens (Cross-arm
findings 5 and 7).
...
CodeBLEU gap runs ≈0.083 at 1K → ≈0.067 at 5K → ≈0.033 at 15K → **≈0** at the full 52K split (A 0.477
...
both arms** (every prediction fails to parse — a hard floor, not just a low score) and climbs to
88–92% by 52K.
...
stand in for execution when both are available.
...
2. **Exact match is genuinely ~zero (0–4 of 6,545) — and that is a real property, not a bug.** The
   ...
   whitespace-removed) — **all return exactly 3**. Predictions are complete, well-formed methods
...
7. **Execution is the discriminating lens — only the LLM arms fix real bugs, and the T5 arms' 0% is a
   ...
   **whole-file-vs-method domain mismatch**: T5 was trained and evaluated on CodeXGLUE's *abstracted
   ...
   output never compiles. The instruction-tuned Qwen arms, by contrast, generalize — zero-shot (C) or
   with light adaptation (D) — to whole-file input despite never being finetuned on it. This is itself
